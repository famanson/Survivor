...
public class FrameBufferObject {
    ...
    public FrameBufferObject(Dimension size) {
        ...
        if (size!=null) {
            ...
            //# Internal size
            if(!supportNonP2) {
                a_width = next_poweroftwo(size.width);
                a_height = next_poweroftwo(size.height);
            }
            else {
                a_width = size.width;
                a_height = size.height;
            }
        } else {
            IntBuffer viewport = BufferUtils.createIntBuffer(16);
            GL11.glGetInteger(GL11.GL_VIEWPORT, viewport);
            //# Actual size to draw
            this.size = new Dimension(viewport.get(2), viewport.get(3));
            //# Internal size
            if(!supportNonP2) {
                a_width = next_poweroftwo(this.size.width);
                a_height = next_poweroftwo(this.size.height);
            }
            else {
                a_width = this.size.width;
                a_height = this.size.height;
            }
        }
        fbo = null;
        depthbuffer = null;
    ...
    public void render(float alpha) {
        if(!enabled) {
            ...
            GL11.glVertex2f(size.width, 0.0f);
            GL11.glTexCoord2f(1.0f, 1.0f);
            GL11.glVertex2f(size.width, size.height);
            GL11.glTexCoord2f(0.0f, 1.0f);
            GL11.glVertex2f(0.0f, size.height);
            GL11.glEnd();
